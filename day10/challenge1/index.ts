...
const filterUniqueTrailSpots = (trailSpots: TrailSpot[]): TrailSpot[] => {
  const uniqueTrailSpots = new Map<string, TrailSpot>();

  trailSpots.forEach((trailSpot) => {
    const { key } = trailSpot.coordinate;
    if (!uniqueTrailSpots.has(key)) {
      uniqueTrailSpots.set(key, trailSpot);
    }
  });

  return Array.from(uniqueTrailSpots.values());
};
...
const investigateTrail = (trailHead: TrailSpot, grid: Grid) => {
  ...
  while (
    ...
    spotsToCheck = filterUniqueTrailSpots(newSpotsToCheck);
  }
