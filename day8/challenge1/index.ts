...
const getAntiNodes = (
  ...
  const x1 = a.x < b.x ? a.x - deltaX : a.x + deltaX;
  const y1 = a.y < b.y ? a.y - deltaY : a.y + deltaY;

  const x2 = b.x < a.x ? b.x - deltaX : b.x + deltaX;
  const y2 = b.y < a.y ? b.y - deltaY : b.y + deltaY;

  return [createCoordinate(x1, y1), createCoordinate(x2, y2)].filter(
    (coord) =>
      coord.x >= 0 && coord.x <= maxX && coord.y >= 0 && coord.y <= maxY
  );
};
